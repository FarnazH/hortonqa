...
def test_load_chgcar_water():
    ...
    assert abs(mol.coordinates[
               1] - np.array([0.074983 * 15 + 0.903122 * 1, 0.903122 * 15, 0.000000]) * angstrom).max() < 1e-10
...
def test_load_poscar_water():
    ...
    assert (mol.numbers == [8, 1, 1]).all()
    assert abs(
        mol.coordinates[1] - np.array([0.074983 * 15, 0.903122 * 15, 0.000000]) * angstrom).max() < 1e-10
    assert abs(mol.cell.volume - (15 * angstrom) ** 3) < 1e-10
