...
class HirshfeldMixin(object):
    ...
    def do_dispersion(self):
        if self.lmax < 3:
            if log.do_warning:
                log.warn(
                    'Skipping the computation of dispersion coefficients because lmax=%i<3' % self.lmax)
            return
        ...
        volume_ratios, new_volume_ratios = self._cache.load(
            'volume_ratios', alloc=self.natom, tags='o')
